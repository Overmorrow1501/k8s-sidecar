...
parser.add_argument(
    ...
    help="path to file containing basic-auth username for REQ. This takes precedence over the environment variable REQ_USERNAME",
)
parser.add_argument(
    "--req-password-file",
    type=str,
    metavar=argparse.REMAINDER,
    help="path to file containing basic-auth password for REQ. This takes precedence over the environment variable REQ_PASSWORD",
)
...
REQ_RETRY_TOTAL = (
    5 if os.getenv("REQ_RETRY_TOTAL") is None else int(os.getenv("REQ_RETRY_TOTAL"))
)
REQ_RETRY_CONNECT = (
    10
    if os.getenv("REQ_RETRY_CONNECT") is None
    else int(os.getenv("REQ_RETRY_CONNECT"))
)
REQ_RETRY_READ = (
    5 if os.getenv("REQ_RETRY_READ") is None else int(os.getenv("REQ_RETRY_READ"))
)
REQ_RETRY_BACKOFF_FACTOR = (
    1.1
    if os.getenv("REQ_RETRY_BACKOFF_FACTOR") is None
    else float(os.getenv("REQ_RETRY_BACKOFF_FACTOR"))
)
REQ_TIMEOUT = (
    10 if os.getenv("REQ_TIMEOUT") is None else float(os.getenv("REQ_TIMEOUT"))
)
...
def write_data_to_file(folder, filename, data, data_type=CONTENT_TYPE_TEXT):
    ...
    if not os.path.exists(folder):
        try:
            ...
            if e.errno == errno.EACCES:
                logger.error(
                    f"Error: insufficient privileges to create {folder}. "
                    f"Skipping {filename}."
                )
                return
    ...
    if os.path.exists(absolute_path):
        ...
        if sha256_hash_new.hexdigest() == sha256_hash_cur.hexdigest():
            logger.debug(
                f"Contents of {filename} haven't changed. Not overwriting existing file"
            )
            return False
...
def fetch_basic_auth_credentials():
    needed_auth = os.getenv("REQ_BASIC_AUTH", default=True)
...
def request(url, method, enable_5xx=False, payload=None):
    ...
    encoding = (
        "latin1"
        if not os.getenv("REQ_BASIC_AUTH_ENCODING")
        else os.getenv("REQ_BASIC_AUTH_ENCODING")
    )
    ...
    try:
        ...
        if method == "GET" or not method:
            res = r.get(
                "%s" % url, auth=auth, timeout=REQ_TIMEOUT, verify=REQ_TLS_VERIFY
            )  # nosem-py/request-without-cert-validation
            logger.info(
                f"Request sent to {url}. Response: {res.status_code} {res.reason} {res.text}"
            )
        elif method == "POST":
            res = r.post(
                "%s" % url,
                ...
                json=payload,
                timeout=REQ_TIMEOUT,
                verify=REQ_TLS_VERIFY,
            )  # nosem-py/request-without-cert-validation
            logger.info(
                f"{payload} sent to {url}. Response: {res.status_code} {res.reason} {res.text}"
            )
        else:
            logger.warning(
                f"Invalid REQ_METHOD: '{method}', please use 'GET' or 'POST'. Doing nothing."
            )
            return
        return res
...
def unique_filename(filename, namespace, resource, resource_name):
    ...
    """
    return (
        "namespace_" + namespace + "." + resource + "_" + resource_name + "." + filename
    )
...
def execute(script_path):
    ...
    try:
        if os.access(script_path, os.X_OK):
            result = subprocess.run([script_path], capture_output=True, check=True)
        else:
            result = subprocess.run(
                ["sh", script_path], capture_output=True, check=True
            )
